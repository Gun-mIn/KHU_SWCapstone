...
def conv3d_model(batch_size, img_size):
...
def c3d_model(batch_size, img_size):
    """ Return the Keras model of the network
    """
    main_input = Input(shape=(batch_size, img_size, img_size, 3), name="main_input")
    # 1st layer group
    x = Conv3D(
        64,
        kernel_size=(3, 3, 3),
    ...
    x = MaxPooling3D(
        pool_size=(2, 2, 2), strides=(2, 2, 2), padding="same", name="pool2"
    )(x)
    # 3rd layer group
    x = Conv3D(
        256,
    ...
    x = MaxPooling3D(
        pool_size=(2, 2, 2), strides=(2, 2, 2), padding="same", name="pool3"
    )(x)
    # 4th layer group
    x = Conv3D(
        512,
    ...
    x = Bidirectional(ConvLSTM2D(32, (3, 3)))(x)
    x = Flatten()(x)
...
def process_batch(video_paths, batch_size, img_size, train=True):
    num = len(video_paths)

    batch = np.zeros((num, batch_size, img_size, img_size, 3), dtype="float32")
    ...
    for i in range(num):
        ...
        if train:
            ...
            for j in range(batch_size):
                ...
                image = cv2.resize(image, (359, 316))
                if is_flip == 1:
                    image = cv2.flip(image, 1)
                batch[i][j][:][:][:] = image[
                                       crop_x: crop_x + img_size, crop_y: crop_y + img_size, :
                                       ]
            labels[i] = label
        else:
            for j in range(batch_size):
...
def generator_train_batch(train_vid_list, batch_size, num_classes, img_size):
...
def generator_val_batch(val_vid_list, batch_size, num_classes, img_size):
...
def main():
    ...
    batch_size = 8  # 32
    img_size = 300
    weights_save_name = "NT_C3D_LSTM_rev{}".format(img_size)

    # Video cropped faces train path
    train_path = ["train_faces/1", "train_faces/0"]
    ...
    if model_name == "c3d":
        model = c3d_model(batch_size=batch_size, img_size=img_size)
    else:
        model = conv3d_model(batch_size=batch_size, img_size=img_size)
    ...
    history = model.fit_generator(
        generator_train_batch(train_vid_list, batch_size, num_classes, img_size),
    ...
    model.save_weights("results/" + weights_save_name + ".hdf5")
